...
1. `_gen_task_list()` — `GET /user` then, for each list in `CLICKUP_LIST_IDS` (Backend > Backlog and Backend > Bug Queue), `GET /list/{list_id}/task` (assignees=me, include_closed=true) — fetches tasks assigned to the user on both lists (id/name/url/status/listName/dateUpdated only, no text), merges them, then filters to `TRACKED_STATUSES` (near-miss statuses surfaced in `unmatchedStatuses` instead of silently dropped). Backlog and Bug Queue use different ClickUp status vocabularies (e.g. "in development"/"shipped" vs "in progress"/"complete"), so `TRACKED_STATUSES` lists both sets.
